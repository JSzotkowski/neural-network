...
if __name__ == '__main__':
    ...
    # net.train_using_nesterov(training_data, epochs, batch_size, 0.3, 0.8, test_data, validation_data)
    ...
    # net = NeuralNetwork(network_layout)
    # net.init_from_json(os.path.join(run_folder_name, 'randomly_initialized_network.txt'))
    # net.train_using_adagrad(training_data, epochs, batch_size, 0.5, test_data, validation_data)
    # net.save_to_file(os.path.join(run_folder_name, 'trained_network_AdaGrad.txt'))
    # # net.save_stats(os.path.join(run_folder_name, 'AdaGrad_stats.txt'))
    # end_time = time()
    # time_log = print_runtime_and_write_to_time_log(time_log, start_time, end_time, 'AdaGrad')

    # Adam - Adaptive moment estimation
    # start_time = time()
    # net = NeuralNetwork(network_layout)
    # net.init_from_json(os.path.join(run_folder_name, 'randomly_initialized_network.txt'))
    # net.train_using_adam(training_data, epochs, batch_size, 4.0, 0.75, 0.999, test_data, validation_data)
    # net.save_to_file(os.path.join(run_folder_name, 'trained_network_Adam.txt'))
    # # net.save_stats(os.path.join(run_folder_name, 'Adam_stats.txt'))
    # end_time = time()
    # time_log = print_runtime_and_write_to_time_log(time_log, start_time, end_time, 'Adam')
